...
def get_record(row):
    ...
    try:
        record = ''
        answer = verify_nan(row['AnswerDescription'])
        if answer:
            record += f"Pergunta: {row['Question']} <br>Resposta: {answer}"
    except:
        return ''
...
todos_arquivos = glob.glob(f'{path_file}/PatientAnamnesis.xlsx')
...
for idx, row in df.iterrows():
    ...
    date_str = row['z_Inserted_Date'][:19]
...
create_log(log_data, log_folder, "log_inserted_record_PatientAnamnesis.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_PatientAnamnesis.xlsx")
