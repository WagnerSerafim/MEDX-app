...
todos_arquivos = glob.glob(f'{path_file}/*atendimento_agenda.xml')
...
for _, row in df.iterrows():
    ...
    patient = exists(session, 0 - row["Paciente"], "Id do Cliente", Contatos)
    if not patient:
        not_inserted_cont += 1
        ...
        if row['Servico'] != "" and row['Servico'] != None and row['Servico'] != 'nan':
            description += f" - {row['Servico']}"

    exists_row = session.query(Agenda).filter(getattr(Agenda, 'Id do Agendamento') == 0 - row["Codigounico"]).first()
    if exists_row:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        ...
        id_scheduling = 0 - row["Codigounico"]
    
    date_str = f'{row["Data"]} {row["Horainicial"]}'
    if is_valid_date(date_str, '%Y-%m-%d %H:%M:%S'):
        start_time = date_str
        start_dt = datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
        end_time = f'{row["Data"]} {row["Horafinal"]}'
    else:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    if row['Profissional'] == 'Benie crispel goldman':
        user = -1022085243
    else:
        user = 1
...
create_log(log_data, log_folder, "log_inserted_atendimento_agenda.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_atendimento_agenda.xlsx")
