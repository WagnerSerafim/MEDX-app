...
df = pd.read_excel(todos_arquivos[0], sheet_name='orientacoes')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    if row['ORIENTACAO'] not in ['', None] and not pd.isna(row['ORIENTACAO']):
        record = f'Orientação:<br>{row['ORIENTACAO']}'
    else:
        not_inserted_cont +=1
        row_dict = row.to_dict()
