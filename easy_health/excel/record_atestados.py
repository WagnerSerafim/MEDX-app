...
df = pd.read_excel(todos_arquivos[0], sheet_name='atestados')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    if row['ATESTADO'] not in ['', None] and not pd.isna(row['ATESTADO']):
        record = f'Atestado/Declaração:<br>{row['ATESTADO']}'
    else:
        not_inserted_cont +=1
        row_dict = row.to_dict()
