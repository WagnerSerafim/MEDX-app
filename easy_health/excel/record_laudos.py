...
df = pd.read_excel(todos_arquivos[0], sheet_name='laudos')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    if row['LAUDO'] not in ['', None] and not pd.isna(row['LAUDO']):
        record = f'Laudo:<br>{row['LAUDO']}'
    else:
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_laudos.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_laudos.xlsx")
