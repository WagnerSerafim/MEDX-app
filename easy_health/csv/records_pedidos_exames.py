...
todos_arquivos = glob.glob(f'{path_file}/pedido_exames*.csv')
...
for idx, row in df.iterrows():
    ...
    if row['PEDIDO EXAME'] not in ['', None] and not pd.isna(row['PEDIDO EXAME']):
        record = f'Pedido de Exame:<br>{row['PEDIDO EXAME']}'
    else:
        not_inserted_cont +=1
        row_dict = row.to_dict()
