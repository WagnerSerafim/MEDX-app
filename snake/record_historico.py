...
todos_arquivos = glob.glob(f'{path_file}/historicomigracao.xlsx')
...
for idx, row in df.iterrows():
    ...
    id_record = verify_nan(row['IdHistoricoMigracao'])
    if id_record in ['', None]:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'IdHistoricoMigracao vazio'
    ...
    record  = verify_nan(row['PalavrasChave'])
    if record in [None, '', 'None'] or pd.isna(record):
        ...
        record = f'{row['historico']}<br><br>{record}'

    date = verify_nan(row['DataAtendimento'])
...
create_log(log_data, log_folder, "log_inserted_record_historicomigracao.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_historicomigracao.xlsx")