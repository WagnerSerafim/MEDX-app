...
def get_record(row):
    ...
    """
    try:
        record = rtf_to_text(row['Historico'])
...
df = pd.read_excel(todos_arquivos[0], sheet_name='Consulta')
df = df.replace('None', '')
...
for _, row in df.iterrows():
    ...
    existing_record = exists(session, row['Seq'], "Id do Histórico", HistoricoClientes)
    if existing_record:
        not_inserted_cont +=1
