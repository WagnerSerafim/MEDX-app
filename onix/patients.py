...
cadastro_file = glob.glob(f'{path_file}/PACIENTE_2*.csv')
extra_file = glob.glob(f'{path_file}/PACIENTE_EXTRA*.csv')
complement_file = glob.glob(f'{path_file}/PACIENTE_COMPL*.csv')

df = pd.read_csv(cadastro_file[0], engine='python', dtype=str, sep=';', quotechar='"', on_bad_lines='skip')
df_extra = pd.read_csv(extra_file[0], engine='python', dtype=str, sep=';', quotechar='"', on_bad_lines='skip')
df_complement = pd.read_csv(complement_file[0], engine='python', dtype=str, sep=';', quotechar='"', on_bad_lines='skip')
...
for idx, row in df_extra.iterrows():
    address = verify_nan(row['DS_ENDERECO'])
    if address != None:
        address += f', {verify_nan(row["NR_LOGRADOURO"])}'
    extra_infos[int(row['CD_PACIENTE'])] = {
        'celular': verify_nan(row['NR_FONE1']),
        'endereco': address,
        'bairro': verify_nan(row['NM_BAIRRO']),
        'cep': verify_nan(row['NR_CEP']),
        'sexo': verify_nan(int(row['TP_SEXO']))
    }

complement_infos = {}
for idx, row in df_complement.iterrows():
    obs_str = verify_nan(row['DS_OBS'])
    if obs_str:
        obs_str = rtf_to_text(obs_str)
    complement_infos[int(row['CD_PACIENTE'])] = {
        'email': verify_nan(row['DS_EMAIL']),
        'observacao': obs_str
    }
for idx, row in df.iterrows():
    ...
    id_patient = verify_nan(row["CD_PACIENTE"])
    if id_patient == None:
        not_inserted_cont +=1
    ...
    extra_infos_patient = extra_infos[id_patient]
    complement_infos_patient = complement_infos[id_patient]

    name = verify_nan(row["NM_PACIENTE"])
    if name == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
    ...
    email = getattr(complement_infos_patient, 'email', None) if complement_infos_patient else None

    try:
        birthday_obj = verify_nan(row["DT_NASC"])
        if birthday_obj == None:
            birthday = '1900-01-01'
        else:
            birthday = datetime.strptime(birthday_obj, '%Y-%m-%d %H:%M:%S.%f').strftime('%Y-%m-%d')
            if not birthday or not is_valid_date(birthday, '%Y-%m-%d'):
                birthday = '1900-01-01'
    except ValueError:
        birthday = '1900-01-01'

    sex = extra_infos_patient['sexo']
    sex = 'F' if sex == 1 else 'M'

    mother = None
    father = None
    rg = verify_nan(row['NR_RG'])
    cpf = limpar_cpf(verify_nan(row['NR_CPF']))
    conjuge = None
    observations = None
    cellphone = limpar_numero(extra_infos_patient['celular'])
    phone = None
    occupation = None
    cep = extra_infos_patient['cep']
    address = extra_infos_patient['endereco']
    complement = None
    neighborhood = extra_infos_patient['bairro']
    city = None
    state = None
