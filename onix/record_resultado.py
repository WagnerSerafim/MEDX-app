...
todos_arquivos = glob.glob(f'{path_file}/ATD_PRONTUARIO_RESULTADO*.json')
prontuario_arquivo = glob.glob(f'{path_file}/ATD_PRONTUARIO_2*.csv')

with open(todos_arquivos[0], 'r', encoding='utf-8') as f:
    df = pd.read_json(f)

df_prontuario = pd.read_csv(prontuario_arquivo[0], engine='python', dtype=str, sep=';', quotechar='"', on_bad_lines='skip')
...
for idx, row in df_prontuario.iterrows():
    pronts['CD_PRONTUARIO'] = row['CD_PACIENTE']
...
for idx, row in df.iterrows():
    ...
    record_id = verify_nan(row['CD_PRONTUARIO_RESULTADO'])
    if record_id == None:
        not_inserted_cont += 1
    ...
    id_patient = pronts.get('CD_PRONTUARIO')
    if id_patient == None:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    record = verify_nan(row['DS_RESULTADO_MEMO'])
    if record == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
    ...
    try:
        date_obj = verify_nan(row["DT_UPDATE"])
        if date_obj == None:
            date = '1900-01-01'
        else:
            # Try ISO format first (2025-11-10T12:27:27.000Z)
            try:
                date = datetime.fromisoformat(date_obj.replace('Z', '+00:00')).strftime('%Y-%m-%d %H:%M:%S')
            except (ValueError, AttributeError):
                # Fallback to original format
                date = datetime.strptime(date_obj, '%Y-%m-%d %H:%M:%S.%f').strftime('%Y-%m-%d %H:%M:%S')
...
create_log(log_data, log_folder, "log_inserted_atd_prontuario_resultado.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_atd_prontuario_resultado.xlsx")
