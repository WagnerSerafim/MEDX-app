...
for idx, row in df.iterrows():
    ...
    id_record = verify_nan(row['codatestado'])
    ...
    record = verify_nan(row['textoAtestado'])
    if record is None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_ATESTADO.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_ATESTADO.xlsx")
