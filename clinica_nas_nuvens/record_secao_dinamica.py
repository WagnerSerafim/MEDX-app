...
todos_arquivos = glob.glob(f'{path_file}/SECAO_DINAMICA_RESPOSTA_NOVO.csv')
prontuario_files = glob.glob(f'{path_file}/PRONTUARIO.csv')
pront_detalhes_files = glob.glob(f'{path_file}/PRONTUARIO_DETALHE.csv')
presc_grupo_files = glob.glob(f'{path_file}/SECAO_DINAMICA.csv')

csv.field_size_limit(1000000)
df = pd.read_csv(todos_arquivos[0], sep=',', encoding='utf-8', quotechar='"', on_bad_lines='skip', dtype=str)
...
for idx, row in df.iterrows():
    ...
    id_grupo = int(verify_nan(row['codsecaodinamica']))
    if id_grupo is None:
        not_inserted_cont += 1
    ...
    if id_detalhe is None:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Id do Detalhe não encontrado em SECAO_DINAMICA'
        row_dict['Timestamp'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        not_inserted_data.append(row_dict)
        continue
    ...
    id_record = verify_nan(row['codresposta'])
    ...
    if not answer:
        not_inserted_cont +=1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Resposta vazio ou inválido'
        row_dict['Timestamp'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        not_inserted_data.append(row_dict)
        continue

    question = verify_nan(row['descricaopergunta'])
    if not question:
        not_inserted_cont +=1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Pergunta vazia ou inválida'
        row_dict['Timestamp'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        not_inserted_data.append(row_dict)
        continue
    
    record = f"{question}<br><br>{answer}"
...
create_log(log_data, log_folder, "log_inserted_record_SECAO_DINAMICA_RESPOSTA_NOVO.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_SECAO_DINAMICA_RESPOSTA_NOVO.xlsx")
