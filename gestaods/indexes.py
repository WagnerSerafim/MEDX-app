...
todos_arquivos = glob.glob(f'{path_file}/pacientes_arquivos.xlsx')
...
for idx, row in df.iterrows():
    ...
    if id_patient in ['', None]:
        ...
        row_dict['Motivo'] = 'Cod Paciente vazio'
        row_dict['TimeStamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        not_inserted_data.append(row_dict)
        continue

    record  = verify_nan(row['Nome Arquivo'])
    ...
    url_arquivo = verify_nan(row['Arquivo'])
    classe = extrair_nome(url_arquivo)
    if len(classe) > 100:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    date = verify_nan(row['Data Cadastro'])
...
create_log(log_data, log_folder, "log_inserted_record_pacientes_arquivos.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_pacientes_arquivos.xlsx")