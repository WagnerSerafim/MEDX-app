...
def get_record(pedido_id, pedido_exames):
    """
    A partir da linha do dataframe, retorna o histórico formatado.
    """
    record = ''
    try:
        pedidos_info = pedido_exames[pedido_id]
        nome = pedidos_info['nome']
        conteudo = pedidos_info['impresso']

        record = f"{nome}: {conteudo}"
    except:
        return ''
...
todos_arquivos = glob.glob(os.path.join(path_file, 'CONSULTA_PEDIDOEXAME_*.csv'))
pedido_exame = glob.glob(os.path.join(path_file, 'PEDIDO_EXAME_[0-9]*.csv'))
...
df_pedidoexame = pd.read_csv(pedido_exame[0], dtype=str, sep=',', encoding='utf-8', quotechar='"')
...
pedido_exames = {}
for _,row in df_pedidoexame.iterrows():
    pedido_exames[row['ID']] = {
        'nome': row['NOME'],
...
for idx, row in df.iterrows():
    ...
    record = get_record(row['PEDIDO_EXAME'], pedido_exames)
    if record == "":
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_pedido_exames.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_pedido_exames.xlsx")
