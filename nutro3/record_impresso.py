...
def get_record(impresso_id, impressos):
    """
    A partir da linha do dataframe, retorna o histórico formatado.
    """
    record = ''
    try:
        impresso_infos = impressos[impresso_id]
        nome = impresso_infos['nome']
        impresso = impresso_infos['impresso']
...
todos_arquivos = glob.glob(os.path.join(path_file, 'CONSULTA_IMPRESSO_*.csv'))
impresso = glob.glob(os.path.join(path_file, 'IMPRESSO_[0-9]*.csv'))
...
df_impressos = pd.read_csv(impresso[0], dtype=str, sep=',', encoding='utf-8', quotechar='"')
...
impressos = {}
for _,row in df_impressos.iterrows():
    impressos[row['ID']] = {
        'nome': row['NOME'],
...
for idx, row in df.iterrows():
    ...
    record = get_record(row['IMPRESSO'], impressos)
    if record == "":
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_impressos.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_impressos.xlsx")
