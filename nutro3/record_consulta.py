...
def get_record(row):
    ...
    record = ''
    try:
        content = verify_nan(row['PRONTUARIO'])
        idade = verify_nan(row['IDADE'])
        peso = verify_nan(row['PESO'])
        estatura = verify_nan(row['ESTATURA'])
        imc = verify_nan(row['IMC'])
        peso_alvo = verify_nan(row['PESO_ALVO'])

        if idade:
            record += f"Idade: {idade} anos\n"
        if peso:
            record += f"Peso: {peso} kg\n"
        if estatura:
            record += f"Estatura: {estatura} cm\n"
        if imc:
            record += f"IMC: {imc}\n"
        if peso_alvo:
            record += f"Peso Alvo: {peso_alvo} kg\n"
        if content:    
            record += f"\n{content}"
    except:
        return ''
...
todos_arquivos = glob.glob(os.path.join(path_file, 'CONSULTA_[0-9]*.csv'))
paciente_consultas = glob.glob(os.path.join(path_file, 'PACIENTE_CONSULTAS*.csv'))
...
for idx, row in df.iterrows():
    ...
    id_patient = pacientes.get(row['ID'], None)
    if id_patient == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
