...
def generate_valid_data(session_src, session_dest, ContatosSrc, ContatosDest):
    last_id = -99999999999
    total_validated = 0
...
with SessionDest() as temp_session:
    try:
        buffer = []
        total = 0
        for patient_data in generate_valid_data(session_src, temp_session, ContatosSrc, ContatosDest):
            ...
            if len(buffer) == batch_size:
                ...
                total += len(buffer)
                print(f"Inseridos: {total} registros")
                buffer.clear()
        ...
        if buffer:
            ...
            log_inserted.extend(buffer)
            total += len(buffer)
            print(f"Inseridos: {total} registros (100%)")
