...
def get_record(row):
    fields = ["obs"] + [f"historico{i}" for i in range(1, 14)]
    parts = []

    for field in fields:
        value = verify_nan(row.get(field))
...
df = pd.read_excel(cadastro_file[0], engine='openpyxl', sheet_name='Atendimentos', dtype=str)
...
for idx, row in df.iterrows():
    ...
    record_id = verify_nan(row['atendimento_id'])
    if record_id == None:
        not_inserted_cont += 1
    ...
    try:
        date_obj = verify_nan(row["atenderde"])
        if date_obj == None:
            date = '1900-01-01'
        else:
...
create_log(log_data, log_folder, "log_inserted_records_Atendimentos.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_records_Atendimentos.xlsx")