...
cadastro_file = glob.glob(f'{path_file}/09092025153612.csv')

df = pd.read_csv(cadastro_file[0], engine='python', dtype=str, sep=';', quotechar='"', encoding='latin1', on_bad_lines='skip')
...
for idx, row in df.iterrows():
    ...
    id_patient = verify_nan(row["PAC_CODIGO"])
    if id_patient == None:
        not_inserted_cont +=1
    ...
    email = verify_nan(row["PAC_EMAIL"])

    try:
        birthday_obj = verify_nan(row["PAC_DATNASC"])
    ...
    sex = verify_nan(row['PAC_SEXO'])
    sex = 'F' if sex == 'FEMININO' else 'M'

    mother = None
    father = None
    rg = None
    cpf = limpar_cpf(verify_nan(row['PAC_CPF']))
    conjuge = None
    observations = None
    cellphone = limpar_numero(verify_nan(row['PAC_CELULAR']))
    phone = verify_nan(row['PAC_TELEFONE'])
    occupation = verify_nan(row['PAC_PROFISSAO'])
    cep = verify_nan(row['PAC_CEP'])
    address = verify_nan(row['PAC_ENDERECO'])
    complement = None
    neighborhood = verify_nan(row['PAC_BAIRRO'])
    city = row['PAC_CIDADE']
    state = None
