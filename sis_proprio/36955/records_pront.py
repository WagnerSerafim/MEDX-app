...
SOURCE_PATIENT_COLUMN = "Paciente"
SOURCE_CREATION_DATE_COLUMN = "Creation Date"
SOURCE_MODIFIED_DATE_COLUMN = "Modified Date"

SKIPPED_HISTORY_COLUMNS = {
    SOURCE_PATIENT_COLUMN,
    SOURCE_CREATION_DATE_COLUMN,
    SOURCE_MODIFIED_DATE_COLUMN,
    "Slug",
    "Creator",
    "unique id",
}
...
def build_history_text(row: dict) -> str | None:
    ...
    for column, raw_value in row.items():
        ...
        if column_name in SKIPPED_HISTORY_COLUMNS:
            continue

        value = normalize_record_text(raw_value)
        if value is None:
            continue

        if "<br>" in value:
            sections.append(f"{column_name}:<br>{value}")
        else:
            sections.append(f"{column_name}: {value}")
...
def build_base_candidate(row: dict) -> tuple[dict | None, dict | None]:
    ...
    if record_text is None:
        return None, {"Timestamp": timestamp, "Motivo": "Historico resumido vazio"}

    patient_name = normalize_contact_name(row.get(SOURCE_PATIENT_COLUMN))
    ...
    return {
        ...
        "date": normalize_datetime(row.get(SOURCE_CREATION_DATE_COLUMN)),
    }, None
...
def process_chunk(
    ...
    required_columns = [SOURCE_PATIENT_COLUMN, SOURCE_CREATION_DATE_COLUMN]
    missing_columns = [column for column in required_columns if column not in df_chunk.columns]
    if missing_columns:
    ...
    name_to_client_ids = fetch_client_ids_by_name(patient_names_to_resolve)
    to_insert: list[tuple[dict, dict]] = []

    for row_dict, candidate in base_candidates:
        patient_name = candidate["patient_name"]
        client_ids = name_to_client_ids.get(patient_name, [])
...
def resolve_csv_file(base_path: str) -> str:
    patterns = [
        "Prontuários Resumidos.csv",
        "Prontuarios Resumidos.csv",
        "*Prontuários*Resumidos*.csv",
        "*Prontuarios*Resumidos*.csv",
    ]
    ...
    raise FileNotFoundError(
        f"Nenhum arquivo encontrado com o nome 'Prontuários Resumidos.csv' em: {base_path}"
    )
...
def build_csv_iter(csv_file: str):
    ...
    for encoding in encodings_to_try:
        try:
            ...
            first_chunk.columns = [str(column).strip() for column in first_chunk.columns]
...
def main() -> None:
    print("Sucesso! Inicializando migracao de Prontuarios Resumidos...")
    ...
    inserted_log_file = log_folder / "log_inserted_records_prontuarios_resumidos.jsonl"
    rejected_log_file = log_folder / "log_not_inserted_records_prontuarios_resumidos.jsonl"
    error_log_file = log_folder / "log_insert_errors_records_prontuarios_resumidos.jsonl"
    ...
    print(f"{inserted_total} novos historicos resumidos foram inseridos com sucesso!")
    if rejected_total > 0:
        print(
            f"{rejected_total} historicos resumidos nao foram inseridos, "
