...
def get_record(json_consulta,resp):

    perguntas = ["Intercorrências","Tratamentos Anteriores", "Plano", "Conduta", "Tratamentos Anteriores"]
    record = ""
    date = "1900-01-01 00:00"
    codp = None

    for consulta in json_consulta:
        if resp["codcon"] == consulta["codcon"]:
            pergunta = perguntas[resp["id_pergunta"]-1]
            record = f"<br>{pergunta}: <br>{resp["resposta"]}"
...
json_file = input("Informe a pasta do arquivo JSON respostasperguntas: ")
with open(json_file, 'r', encoding='utf-8') as file:
...
for dict in json_data:
    ...
    if dict["resposta"] == '' or dict["resposta"] == '<br>' or dict["resposta"][:3] == "<!--":
        continue
...
log_file_path = os.path.join(log_folder, "record_perguntasrespostas_log.xlsx")
