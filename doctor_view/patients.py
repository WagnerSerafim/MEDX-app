...
cadastro_file = glob.glob(f'{path_file}/VW_PESSOA_FIS*.json')
...
for idx, row in df.iterrows():
    ...
    try:
        birthday_obj = verify_nan(row["DTNASC"])
    ...
    mother = None
    father = None
    address = verify_nan(row['ENDERECO_1'])
    neighborhood = verify_nan(row['BAIRRO_1'])
    city = verify_nan(row['CIDADE_1'])
    cellphone = verify_nan(row['TELEFONE_1'])
    phone = None
    rg = verify_nan(limpar_numero(row['RG']))
    cpf = verify_nan(limpar_cpf(row['CPF']))
    conjuge = None
    observations = None
    occupation = verify_nan(row['PROFISSAO'])
    cep = verify_nan(limpar_numero(row['CEP_1']))   
    complement = None
    state = None
...
create_log(log_data, log_folder, "log_inserted_VW_PESSOA_FIS.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_VW_PESSOA_FIS.xlsx")
