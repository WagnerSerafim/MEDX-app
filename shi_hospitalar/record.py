...
schedule_file = glob.glob(f'{path_file}/Agendamento_fixed.csv')
...
df_schedule = pd.read_csv(schedule_file[0], sep=';', encoding='utf-8', dtype=str, quotechar='"')
...
for _, row in df_schedule.iterrows():
    ...
    pront = str(row["PRONT"])
    if codigo.isdigit() and pront.isdigit():
        schedule_lookup[codigo] = pront

print(df_schedule[['CODIGO', 'PRONT']])
...
for _, row in df.iterrows():
    ...
    if row['CD_AGEND'] in schedule_lookup:
        id_patient = schedule_lookup[row['CD_AGEND']]
    else:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Id do paciente não encontrado no arquivo Agendamento.xml'
    ...
    if record == '':
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Registro vazio'
        not_inserted_data.append(row_dict)
        continue
