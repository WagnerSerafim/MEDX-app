...
todos_arquivos = glob.glob(f'{path_file}/records_file.csv')
record_path = glob.glob(f'{path_file}/records.csv')

df = pd.read_csv(todos_arquivos[0], sep=',')
df = df.replace('None', '')

df_records = pd.read_csv(record_path[0], sep=',')
...
create_log(log_data, log_folder, "log_inserted_records_file.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_records_file.xlsx")