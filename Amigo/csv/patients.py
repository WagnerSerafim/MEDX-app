...
def get_adress(row):
    street = verify_nan(row['address_address'])
    number = verify_nan(row['address_number'])
...
cadastro_file = glob.glob(f'{path_file}/patients.csv')
...
for idx, row in df.iterrows():
    ...
    birthday = verify_nan(row["born"])
    if not birthday or not is_valid_date(birthday, '%Y-%m-%d %H:%M:%S'):
        birthday = '1900-01-01'

    sex = verify_nan(row['gender'])
    sex = 'F' if sex == 'Feminino' else 'M'

    mother = verify_nan(row['mother_name'])
    father = verify_nan(row['father_name'])
    rg = verify_nan(row['rg'])
    cpf = limpar_cpf(verify_nan(row['cpf']))
    conjuge = verify_nan(row['husband_name'])
    observations = f"{verify_nan(row['observation'])} {verify_nan(row['notes'])}"
    if observations in ['None None', 'None', '', ' ']:
        observations = None
    
    cellphone = limpar_numero(verify_nan(row['contact_cellphone']))
    phone = verify_nan(row['contact_phone_home'])
    cep = limpar_numero(verify_nan(row['address_cep']))
    address = get_adress(row)
    complement = verify_nan(row['address_complement'])
    neighborhood = verify_nan(row['address_district'])
    city = verify_nan(row['address_city'])
    state = verify_nan(row['address_state'])
    occupation = verify_nan(row['jobrole'])
