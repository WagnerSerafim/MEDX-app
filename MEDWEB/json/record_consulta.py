...
todos_arquivos = glob.glob(f'{path_file}/consulta.json')
...
for idx, row in df.iterrows():
    ...
    record = verify_nan(row['sintomas'])
    if record == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_consulta.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_consulta.xlsx")
