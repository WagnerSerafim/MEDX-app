...
todos_arquivos = glob.glob(f'{path_file}/HISTORIC.xml')
...
for _, row in df.iterrows():
    ...
    if row['CodPaciente'] in ['None', None, ''] or pd.isna(row['CodPaciente']):
        not_inserted_cont += 1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_HISTORIC.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_HISTORIC.xlsx")