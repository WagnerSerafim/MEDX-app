...
for _, row in df.iterrows():
    ...
    year = str(row['AnoNascimento'])
    month = str(row['MesDiaNascimento'])[:2]
    day = str(row['MesDiaNascimento'])[2:]
    ...
    if row['CodConvenio'] in [None, 'None', ''] or pd.isna(row['CodConvenio']):
        id_convenio = ''
    else:
        id_convenio = row['CodConvenio']
    ...
    setattr(new_patient, "Cônjugue", truncate_value(partner, 50))
    setattr(new_patient, "Id do Convênio", id_convenio)
