...
todos_arquivos = glob.glob(f'{path_file}/SORAIA.xml')
...
for _, row in df.iterrows():
    ...
    if id_patient in ['None', None, ''] or pd.isna(row['CodPaciente']):
        not_inserted_cont +=1
    ...
    if row['CodAgendamento'] in ['None', None, ''] or pd.isna(row['CodAgendamento']):
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Id vazio ou inválido'
        not_inserted_data.append(row_dict)
        continue
    else:
        schedule_id = row['CodAgendamento']

    exists_row = exists(session, schedule_id, "Id do Agendamento", Agenda)
    if exists_row:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    if '_' in date_str:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Data contém "_"'
        not_inserted_data.append(row_dict)
        continue

    date_str = f'{row["Data"][:4]}-{row["Data"][4:6]}-{row['Data'][6:8]}'
    hour_str = str(row['Hora'])
    date = f'{date_str} {row['Hora']}'

    if is_valid_date(date, '%Y-%m-%d %H:%M'):
        start_time = date
    ...
    description = f'{row['NomePaciente']} {'Observacao'}'
    ...
    setattr(new_schedulling, "Id do Agendamento", schedule_id)
...
create_log(log_data, log_folder, "log_inserted_scheduling_SORAIA.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_scheduling_SORAIA.xlsx")
