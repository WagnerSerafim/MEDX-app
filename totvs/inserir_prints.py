...
def save_jsonl_log():
    ...
    log_path = os.path.join(log_folder, f"migracacao_prints_{execution_start.strftime('%Y%m%d_%H%M%S')}.jsonl")
...
# ===== BACKUP DA TABELA =====
print("Gerando backup da tabela [Histórico de Clientes]...")
log_jsonl_entry("BACKUP", "Iniciando backup da tabela [Histórico de Clientes]")
...
try:
    ...
    backup_path = os.path.join(log_folder, f"backup_historico_{execution_start.strftime('%Y%m%d_%H%M%S')}.xlsx")
    backup_df.to_excel(backup_path, index=False)
    
    log_jsonl_entry("BACKUP", "Backup concluído com sucesso", {
        "total_registros": len(backup_df),
        "caminho_arquivo": backup_path
    })
    print(f"Backup concluído: {len(backup_df)} registros salvos")
except Exception as e:
    log_jsonl_entry("ERRO", "Falha ao gerar backup", {"erro": str(e)})
...
log_jsonl_entry("DEDUPLICACAO", "Lendo backup para preparar conjunto de deduplicação")

# Criar um conjunto com as combinações (Histórico, Data, Id do Cliente) do backup
...
print("Lendo arquivo de prints...")
log_jsonl_entry("ENTRADA", "Lendo arquivo Excel com registros de prints")

extension_file = glob.glob(f'{path_file}/prints*.xlsx')
if not extension_file:
    log_jsonl_entry("ERRO", "Nenhum arquivo prints*.xlsx encontrado", {"caminho_busca": path_file})
    print(f"Erro: Nenhum arquivo prints*.xlsx encontrado em {path_file}")
    session.close()
    save_jsonl_log()
    raise FileNotFoundError(f"Nenhum arquivo prints*.xlsx encontrado em {path_file}")
...
log_jsonl_entry("MIGRACAO", "Iniciando migração de históricos")
...
for idx, row in df.iterrows():
    if idx % 1000 == 0 or idx == len(df) - 1:
        ...
        status = f"Processados: {idx}/{len(df)} | Inseridos: {inserted_count} | Duplicados: {skipped_duplicates} | Inválidos: {skipped_invalid} | Concluído: {progress}%"
        print(status)
    ...
    # Validação 2: Verificar conteúdo do histórico
...
summary = {
    ...
    "arquivo_prints": extension_file[0],
    "arquivo_backup": backup_path if 'backup_path' in locals() else "Não gerado",
