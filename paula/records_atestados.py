...
SOURCE_FILE_NAME = "medx_paciente_atestado.csv"
...
def build_history_html(row):
    title = clean_spaces(row.get("strTitulo"))
    professional = clean_spaces(row.get("strFuncionario"))
    body = paragraphize_text(row.get("strTexto"))

    parts = []
    if title:
        parts.append(f"<h3>{escape(title)}</h3>")
    else:
        parts.append("<h3>Atestado</h3>")
...
def make_payload(row):
    ...
    payload = {
        ...
        "Id do Usuário": 0,
        "Data": normalize_datetime(row.get("dataInsert")),
...
def prepare_rows(df, existing_patient_ids, existing_record_ids):
    ...
    for _, row in df.iterrows():
        ...
        text = clean_value(row.get("strTexto"))
        ...
        if not text:
            not_inserted_log.append(row_to_log(row, payload, "Atestado sem texto"))
            continue

        if source_id in seen_record_ids:
            not_inserted_log.append(row_to_log(row, payload, "Atestado duplicado no CSV"))
            continue

        seen_record_ids.add(source_id)

        if source_id in existing_record_ids:
            not_inserted_log.append(row_to_log(row, payload, "Atestado já existe no banco"))
            continue
...
def insert_payloads(session, historico_tbl, payloads):
    ...
    for batch_number, batch in enumerate(iter_chunks(payloads, BATCH_SIZE), start=1):
        try:
            ...
            print(f"Lote {batch_number}: {len(batch)} atestados inseridos.")
...
def main():
    print("=== Migração segura de atestados - Dra. Paula ===")
    path_file = input("Informe o caminho da pasta que contém o medx_paciente_atestado.csv: ").strip()
    log_folder = Path(path_file)
    df = load_source_csv(log_folder)

    print(f"CSV carregado: {len(df)} atestados.")
    ...
    try:
        ...
        print(f"Atestados já existentes no banco: {len(existing_record_ids)}")
        ...
        write_json_log(preview_log, log_folder, "log_preview_records_atestados.json")
        if not_inserted_log:
            write_json_log(not_inserted_log, log_folder, "log_not_inserted_records_atestados.json")
        ...
        write_json_log(inserted_log, log_folder, "log_inserted_records_atestados.json")
        write_json_log(not_inserted_log, log_folder, "log_not_inserted_records_atestados.json")
