...
def get_adress(row):
    street = verify_nan(row['strEndereco'])
...
for idx, row in df.iterrows():
    ...
    name = verify_nan(row["strCliente"])
    if name == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
    ...
    birthday = verify_nan(row["datNascimento"])
    # if not birthday or not is_valid_date(birthday, '%Y-%m-%d %H:%M:%S'):
    #     birthday = '1900-01-01'

    email = verify_nan(row["strEmail"])
    sex = verify_nan(row['strSexo'])
    sex = 'F' if sex == 'FEMININO' else 'M'

    mother = verify_nan(row['strMae'])
    father = verify_nan(row['strPai'])
    rg = verify_nan(row['strRG'])
    cpf = limpar_cpf(verify_nan(row['strCPF']))
    conjuge = None
    observations = None
    
    cellphone = limpar_numero(verify_nan(row['strCelular']))
    phone = None
    cep = limpar_numero(verify_nan(row['strCep']))
    address = get_adress(row)
    complement = verify_nan(row['strComplemento'])
    neighborhood = verify_nan(row['strBairro'])
    city = None
    state = None
    occupation = None
