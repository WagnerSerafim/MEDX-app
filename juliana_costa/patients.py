...
df = pd.read_csv(cadastro_file[0], sep=',', engine='python', quotechar='"', encoding='utf-8')
...
for idx, row in df.iterrows():
    ...
    birthday_str = verify_nan(row["Data de Nascimento"])
    ...
    father = None
    rg = None
    cpf = limpar_cpf(verify_nan(row['Documento']))
    conjuge = None
