...
for idx, row in df.iterrows():
    ...
    user = row['Agenda']
    if user == 'DRA POLLYANA':
        user = 1
    elif user == 'DRA JULIANA':
        user = 1111272341
    else:
        not_inserted_cont += 1
    ...
    patient_obj = exists(session, id_patient, "Referências", Contatos)
    if not patient_obj:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    if not patient:
        ...
        not_inserted_data.append(row_dict)
        continue
    observation = verify_nan(row["Observações internas"])
    ...
    hour_end_obj = verify_nan(row['Horário de término'])
...
create_log(log_data, log_folder, "log_inserted_schedule_Agendas.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_schedule_Agendas.xlsx")
