...
todos_arquivos = glob.glob(f'{path_file}/t_agendaconsultas.csv')
...
for idx, row in df.iterrows():
    ...
    id_patient = verify_nan(row["numeroprontuario"])
    if id_patient == None:
        not_inserted_cont += 1
    ...
    observation = verify_nan(row['anotacoes'])
    if observation == None:
        observation = ''

    patient = verify_nan(row["nomepaciente"])
    if not patient:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    hour_obj = verify_nan(row['hora'])
    if date_obj == None or hour_obj == None:
        not_inserted_cont += 1
        row_dict = row.to_dict()
