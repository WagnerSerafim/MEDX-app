...
todos_arquivos = glob.glob(f'{path_file}/historico_2.xlsx')
...
for idx, row in df.iterrows():
    ...
    record = verify_nan(row['tbReceituario'])
    if record == None:
        not_inserted_cont += 1
        row_dict = row.to_dict()
    ...
    date = verify_nan(row['tbDataHora'])
...
create_log(log_data, log_folder, "log_inserted_historico_2.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_historico_2.xlsx")
