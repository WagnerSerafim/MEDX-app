...
todos_arquivos = glob.glob(f'{path_file}/EvolucaoPadrao.csv')

df = pd.read_csv(todos_arquivos[0], dtype=str, encoding='latin1', sep=';', quotechar='"')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    id_patient = verify_nan(row['tbNumFicha'])
    if id_patient == None:
        not_inserted_cont += 1
    ...
    record = verify_nan(row['tbDescricao'])
    if record == None:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'Descrição do Histórico vazia'
    ...
    date = datetime.combine(date_obj.date(), hour_obj.time())
...
create_log(log_data, log_folder, "log_inserted_EvolucaoPadrao.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_EvolucaoPadrao.xlsx")
