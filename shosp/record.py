...
todos_arquivos = glob.glob(f'{path_file}/prontuario-formulario-*.csv')
...
for _, row in df.iterrows():
    ...
    if row['Resposta Texto'] in [None, '', 'None'] or pd.isna(row['Resposta Texto']):
        not_inserted_cont +=1
        row_dict = row.to_dict()
        ...
        record = f'{row['Descricao']}:<br>{row['Resposta Texto']}'
...
create_log(log_data, log_folder, "log_inserted_record_prontuario-formulario.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_prontuario-formulario.xlsx")
