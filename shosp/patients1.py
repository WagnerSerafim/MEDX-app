...
extension_file = glob.glob(f'{path_file}/*cadastro_paciente*.csv')

df = pd.read_csv(extension_file[0], sep=';', engine='python', encoding='latin1')
print(df.head())
...
for idx, row in df.iterrows():
    ...
    if row["Pront."] in [None, '', 'None'] or pd.isna(row["Pront."]):
        not_inserted_cont +=1
    ...
    date_str = verify_nan(row['Dt. Nasc.'])
    if date_str is None:
        birthday = '1900-01-01'
    else:
    ...
    mother = verify_nan(row['Nome mae'])
...
create_log(log_data, log_folder, "log_inserted_cadastro_paciente.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_cadastro_paciente.xlsx")
