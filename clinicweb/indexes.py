...
def build_insert_payload(row_dict: dict) -> tuple[dict | None, dict | None]:
    ...
    data_final = row_dict.get("DATA")
    classe = row_dict.get("imagem")
...
async def main():
    ...
    arquivos = glob.glob(f"{path_file}/arquivos*.csv")
    if not arquivos:
        raise FileNotFoundError(
            f"Nenhum arquivo encontrado com o padrão 'arquivos*.csv' em: {path_file}"
        )
    ...
    inserted_log_file = log_folder / "log_inserted_record_arquivos.jsonl"
    rejected_log_file = log_folder / "log_not_inserted_record_arquivos.jsonl"
    error_log_file = log_folder / "log_insert_errors_record_arquivos.jsonl"
