...
df = pd.read_excel(todos_arquivos[0], sheet_name='TEX_PAC')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    record_id = verify_nan(row['ID_Text'])
    if not record_id:
        not_inserted_cont += 1
        row_dict = row.to_dict()
        row_dict['Motivo'] = 'ID_Text vazio'
    ...
    record = verify_nan(row['TextodoPaciente'])
    if record == None:
        not_inserted_cont +=1
        row_dict = row.to_dict()
...
create_log(log_data, log_folder, "log_inserted_record_TEX_PAC.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_TEX_PAC.xlsx")
