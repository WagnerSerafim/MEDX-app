...
df = pd.read_excel(todos_arquivos[0], sheet_name='ANAM_PAC')
df = df.replace('None', '')
...
for idx, row in df.iterrows():
    ...
    if is_valid_date(row['Anam_Date'], '%Y-%m-%d %H:%M:%S'):
        date = row['Anam_Date']
    else:
...
create_log(log_data, log_folder, "log_inserted_record_ANAM_PAC.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_ANAM_PAC.xlsx")
