...
todos_arquivos = glob.glob(f'{path_file}/diagnosticos.csv')
...
for idx, row in df.iterrows():
    ...
    if row['DIAGNOSTICO'] not in ['', None] and not pd.isna(row['DIAGNOSTICO']):
        record = f'Diagnóstico:<br>{row['DIAGNOSTICO']}'
    else:
        not_inserted_cont +=1
...
create_log(log_data, log_folder, "log_inserted_record_diagnosticos.xlsx")
create_log(not_inserted_data, log_folder, "log_not_inserted_record_diagnosticos.xlsx")
