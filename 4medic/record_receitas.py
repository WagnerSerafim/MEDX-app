...
for idx, row in df.iterrows():
    ...
    if row['HTML_RECEITA'] not in ['', None] and not pd.isna(row['HTML_RECEITA']):
        decoded_url = urllib.parse.unquote_plus(row['HTML_RECEITA'])
        decoded_html = html.unescape(decoded_url)
        record = f'Receita:<br>{decoded_html}'
    else:
        not_inserted_cont +=1
    ...
    if is_valid_date(row['DATA_RECEITA'], '%Y-%m-%d %H:%M:%S'):
        date = row['DATA_RECEITA']
