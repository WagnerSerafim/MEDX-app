...
todos_arquivos = glob.glob(f'{path_file}/impressos.csv')
...
for idx, row in df.iterrows():
    ...
    # id_record = verify_nan(row['ID_IMPRESSO'])
    ...
    if row['HTML_IMPRESSO'] not in ['', None] and not pd.isna(row['HTML_IMPRESSO']):
        decoded_url = urllib.parse.unquote_plus(row['HTML_IMPRESSO'])
        decoded_html = html.unescape(decoded_url)
        record = f'Impressos:<br>{decoded_html}'
    else:
        not_inserted_cont +=1
        row_dict = row.to_dict()
